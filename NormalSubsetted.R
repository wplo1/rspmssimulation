...
NormalSubset200LL3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-3;bYT<-3.6;aYC<-0.1; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200LM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200LM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200LM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200ML1 #### 
cs <- makeCluster(6)
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-0.5;bYT<-3.9;aYC<-3; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200ML1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200ML3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200ML3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200MM1 #### 
cs <- makeCluster(6)
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-1;bYT<-4;aYC<-1; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200MM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200MM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200MM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200MH1 #### 
cs <- makeCluster(6)
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-3;bYT<-4.2;aYC<-0.5; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200MH1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200MH3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200MH3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200HM1 #### 
cs <- makeCluster(6)
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-1;bYT<-4.9;aYC<-3; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200HM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200HM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200HM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200HH1 #### 
cs <- makeCluster(6)
...
resdf <- foreach(i=1:nsim, .combine = 'rbind') %dopar% {
  ...
  resul$seed<-i
  muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;
  aYT<-1;bYT<-4.9;aYC<-1.25; bYC<-4.9
  ts1<-rnorm(200,mean=muST,sd=sigmaST);ts0<-rnorm(200,mean=muSC,sd=sigmaSC);
  functnorm0<-function(s){rbinom(1,1,1/(1+exp(-aYC*(s-bYC))))};
  functnorm1<-function(s){rbinom(1,1,1/(1+exp(-aYT*(s-bYT))))};
  ty1<-sapply(ts1,functnorm1);ty0<-sapply(ts0,functnorm0);
...
muST<-5; sigmaST<-2; muSC<-4; sigmaSC<-1;aYT<-1;bYT<-4.9;aYC<-1.25; bYC<-4.9
YTYT<-function(x){1/(1+exp(-aYT*(x-bYT)))};YCYC<-function(x){1/(1+exp(-aYC*(x-bYC)))}
STST<-function(x){dnorm(x,mean=muST,sd=sigmaST)};SCSC<-function(x){dnorm(x,mean=muSC,sd=sigmaSC)}
TCTC<-integrate(function(x){YTYT(x)*SCSC(x)},-Inf,Inf)
TTTT<-integrate(function(x){YTYT(x)*STST(x)},-Inf,Inf)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset200HH1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset200HH3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset200HH3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000LL1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000LL3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000LM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000LM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000LM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000ML1 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000ML1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000ML3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000ML3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000MM1 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000MM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000MM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000MM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000MH1 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000MH1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000MH3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000MH3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000HM1 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000HM1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000HM3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000HM3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000HH1 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(1))
NormalSubset1000HH1<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time
#### NormalSubset1000HH3 #### 
cs <- makeCluster(6)
...
PMSPMS<-function(r){(r*RSRS+RSPRSP)^2/((r*RSRS+RSPRSP)^2+(r-1)^2*(RSRS-RSPRSP)^2+(1-RSRS+r*(1-RSPRSP))^2)}
finalperfdf<-finalperf(resdf,RSRS,PMSPMS(3))
NormalSubset1000HH3<-finalperfdf[[1]]
end.time<-Sys.time()
end.time-start.time